...
export class FetchQuestionController {
    constructor(
        ...
        readonly UseCase: NestFetchLatestQuestionsUseCase,
    ) { }
    @Get()
    async handler(
        @Query() query: PaginationQueryParams,
    ): Promise<{ questions: QuestionPresenterResponse[]; count: number }> {
        const response = await this.UseCase.execute({
            limit: query.limit,
            page: query.page,
        });

        const questionsPresenter = response.questions.map(QuestionPresenter.toHTTP);

        return {
            questions: questionsPresenter,
